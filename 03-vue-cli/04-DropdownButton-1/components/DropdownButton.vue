...
- Скрывать меню, при клике вне области. Сначала я хотел реализовать это 
  через простой window.addEventListener со свойством once:true, но
  во время клика на второе меню, первое не скрывалось. Так и не понял, почему

- Можно вынести dropdown__menu в отдельный компонент, добившись изоляции
...
export default {
  ...
  computed: {
    ...
    text() {
      return (
        this.title + (this.currentOption ? ' - ' + this.currentOption.text : '')
      );
    },
    currentOption() {
    ...
    icon() {
      return this.hasIcon && this.currentOption ? this.currentOption.icon : '';
    },
  },
